...
| Info pages | `/faqs/`,`/shipping/`,`/returns/` | `/merch/pages/[slug]` | ⚠️ | 2026-08-03 | FIXED (#640/#641, PR #642): SUPPORT hero + 9-pill tab strip + typography + white bg. Residual: mascot = champion-splash placeholder. **NEW DELTA (merch-gap 2026-08-04):** 3 slugs are FORM-first on the real site, prose stubs for us — `/order-lookup/` (3-field order lookup), `/gift-card-balance/` (2-field balance checker), `/product-validation/` (auth-code entry). Our `order-status`/`gift-card-balance`/`verify-your-product` render prose instead of the form widget. Fidelity-engine to measure the real forms + file a build issue (new MerchSupportForm sub-component; prose slugs stay prose) |
