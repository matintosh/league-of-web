...
|---|---|---|---|---|---|
| Homepage | `/` | `/merch` | ⚠️ #660 #661 | 2026-08-03 | re-verify #2: sale-badge (#644) + no-REFINE (#647) intact (no regression). 2 real header deltas: announcement is a rounded-pill + scrolling MARQUEE (never built despite #590 claiming it) → #660; logo lockup order reversed — real = wordmark THEN dark fist, ours = red-fist THEN wordmark → #661. #659 (nav-typo revert) REJECTED+CLOSED (oscillation: real nav IS uppercase per real-header.png; #659 mismeasured) |
