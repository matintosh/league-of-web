...
**Status:** last updated 2026-08-03 · **9 / 9 page types live 🎉** · **22 components** shipped · mobile pixel-perfect ✅ (#618-#623) · **desktop 1:1 fidelity pass ✅ COMPLETE** — PDP typography+proportions #627/#628 (PR #630) + listing 2-col flush redesign #629 (PR #631: real cards w/ franchise overlay + multi-badge + REFINE across all 4 pages, homepage corrected 4-col→2-col, MerchProduct badge→+badges[]+franchiseLabel). Cart = DRY. Every page measured/verified at BOTH 1280 + 390. **In flight (user-directed):** (1) REAL images — swapping champion-splash for the actual merch.riotgames.com Sanity-CDN hero banners + 8 real products (hotlink via Next remotePatterns + a `merchAssetUrl` helper); (2) independent `/merch/showcase` under the /merch layout (real merch tokens, fixes the tokenless-render bug in shared /showcase).
