...
- 2026-08-17 — REGRESSION SURFACED by the login selfcheck sweep (audit cycle 3, ledger c9ec55f) — the reference-free internal-consistency modality caught what the ref-based diffs missed. **#1001 (P1, status:ready):** `LolClassicLogo` (packages/ui/src/login/lol-classic-logo.tsx) is missing 4 of 14 glyph paths from the source `LolWordmark` (franchise-logos.tsx) — renders "LEAG E LEGEN" in the Classic theme + its showcase (dropped U/O/F/S/D glyphs). **#1002 (P3, status:ready):** raw hex #2a1f0e/#1a1208 in login-notice-banner.showcase.tsx inline styles (#1a1208→--color-login-black; #2a1f0e needs a token). CAVEAT on the 2026-08-10 CONVERGED: it verified the **Current (white) theme** assembled /login (RiotGamesWordmark ✅) — the **Classic-theme LoL wordmark was never a diffed row**, so login is NOT fully 1:1 until #1001 ships. Login drops off CONVERGED → re-verify after #1001/#1002 build. Lesson: theme VARIANTS each need their own row (the assembled diff only covered one theme).
