...
export const playerBannerShowcase: ShowcaseEntry = {
  ...
  area: "lobby",
  description:
    "Vertical banner card for the pre-game lobby. The SELF banner has TWO treatments: the current-client 2025 ornate rank BADGE (issues #471/#475, #494 — pass regaliaSrc = regaliaBannerUrl(tier): a compact ~200×360 badge with a masked red-tinted champion-splash backdrop, a TOP rank-emblem medallion (ornate ring + tier wing-clasps + red heart crest + the handwriting signature inside it + tier gem), the crowned name + title, three mastery medallions, edit/loadout mini-icons, and a prominent red winged wreath foot with a red gem), and the legacy V11 heraldic flag (no regaliaSrc) which teammate / party slots always use. The V11 flag chrome is the REAL client party-banner art (issue #336) — CommunityDragon rcp-fe-lol-parties `banner-filled.png`, a straight-alpha PNG that carries the whole silhouette (scooped-neck top, ornate gold corner scrolls, double-V pointed bottom) plus its gold trim, dropped in as one background <img>. The avatar crest / tier gem / level badge / badges / role row / footer composite on top; content is padded clear of the pointed-bottom band. Crown+name above the shape. Self reuses the same art brightened (its baked medallion ring lives in the unused current-player-banner art, so the live AvatarCrest ring isn't doubled). Deep review #378: the flag panel now renders as the real client's TALL translucent column (h/w≈2.5 — the native banner-filled art is a stubby 1.44 shield; the v11 client stretches it into a column matching the invited flag beside it) at reduced opacity so it reads as a dark navy panel the lobby bg shows through, with the ranked wings scaled up to dominate/frame the medallion and the tier gem enlarged. Widths stay 120px (self) / 96px (teammate). Empty: large grey + circle (~90px). Queue treatment (queueing=true): empty slots show dark circle with blue glow ring; self banner shows asterisk foot glyph. Wing art from CommunityDragon ranked-emblem PNGs. tierGem prop adds a ranked-mini-crest PNG at 12 o'clock on the portrait ring; badges tuple adds 3 circular badge slots below the role row.",
  variants: [
    {
      name: "2025 ornate rank-badge (self) — matintosh / challenger",
      notes:
        "The current-client (2025) self banner (issues #471/#475, rebuilt for #494 to match the reference compact rank BADGE — not a thin tall flag). Supplying regaliaSrc on an isSelf banner switches from the V11 heraldic flag to the compact rank badge (~200×360), top → bottom: a champion splash (championSplashUrl) masked in with a deep-red SR-parties tint (ban-red tokens via color-mix); the TOP rank-emblem medallion — an ornate ring with the tier wing-clasps (wingTier) flanking its sides, a red heart crest at its centre, the handwriting SIGNATURE drawn inside the heart, and a tier gem (tierGem) at its foot; the crowned summoner name (gold-cream) + captain crown; a title line (\"Final Boss Faker\", italic grey); three circular mastery medallions (masteryCrestUrl); edit/loadout mini-icons; and a prominent RED WINGED WREATH foot (the tier wings enlarged + red-tinted over the regalia banner's gold-edged base, red gem at centre). FONT-SUBSTITUTION divergence: the real client renders a bespoke signature image (no extractable asset) — the signature here is a Dancing Script web-font stand-in (font-signature token), an intentional named divergence, not a pixel match.",
