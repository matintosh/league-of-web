...
export const topNavbarShowcase: ShowcaseEntry = {
  ...
  area: "chrome",
  description:
    "The slim top navigation bar — play CTA on the left, nav items in the center, currency and controls on the right. The band is a dark art-bleed scrim (translucent hextech-black letting the home key-art show through) with a frosted-glass backdrop blur (#508) so the splash behind it reads as soft frosted glass, not a sharp image. Current era (#384/#386): the right side carries a menu-access icon cluster + stacked currency with an RP top-up disc. Player identity lives in the social-rail ProfileChip / top-right profile chip, not in the band. (In the showcase the band sits on a flat surface, so the blur is only visible in-app over the home splash.)",
