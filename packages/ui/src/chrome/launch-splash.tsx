...
export interface LaunchSplashProps {
  /**
   * Source URL of the ident video (webm with alpha). The real client plays
   * the Riot Games ident animation on launch; supply the served asset path
   * here, e.g. "/media/riot-splash-jinx.webm". The clip carries a native
   * alpha channel, so it composites directly over the dark backdrop.
   */
  videoSrc: string;
  /**
 ...
 *
 * Behaviour:
 * - The ident webm ships a NATIVE alpha channel — it renders directly over a
 *   flat hextech-black field (`object-contain`, once, no loop), blending with
 *   the client beneath. No pixel processing is involved.
...
export function LaunchSplash({ videoSrc, onFinished, skipLabel = "Click anywhere or press Escape to skip" }: LaunchSplashProps) {
  ...
  return (
    <div
      ...
      style={{
        // Flat Hextech field so the splash blends with the client beneath;
        // the alpha-webm ident composites transparently on top.
