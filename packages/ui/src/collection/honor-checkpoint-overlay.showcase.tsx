...
export const honorCheckpointOverlayShowcase: ShowcaseEntry = {
  ...
  area: "collection",
  description:
    "The in-client Honor celebration full-screen overlay — 'CHECKPOINT REACHED', 'HONOR LEVEL UP', and 'HONOR UNLOCKED' (all one layout, different title + crest video). Atmospheric dark-forest gradient backdrop, top/bottom gold hairline rules, font-display uppercase title (text-gold-cream), muted subtitle, a centered animated honor crest, and a gold OK button. Crest video is an intro → loop → outro state machine layered over a static SVG crest glyph, so any missing/broken/slow clip leaves the static crest + gradient intact. Dismiss (OK, backdrop click, or Escape) starts the ~300ms fade immediately — the outro (if any) plays underneath during the fade but never delays the handoff — then fires onFinished once on a single idempotent path. Unlike a skip-on-mount celebration, this surface has an OK button — under prefers-reduced-motion it still renders in full (static crest + gradient, no video) and waits for an explicit dismiss; it never auto-closes. Presentational: page/demo supplies crest/backdrop URLs from @low/fixtures (honorCheckpointVideoUrl / honorLevelUpVideoUrl / honorUnlockVideoUrl / honorTransitionVideoUrl / honorVotingBgVideoUrl). Not wired into a live flow — no honor trigger exists yet; showcase-driven today.",
  referenceNote:
  ...
  variants: [
    {
      ...
      notes:
        "Pick a celebration to play its overlay in a framed stage. Each play key-remounts the overlay so the intro → loop restarts from frame 0; a counter shows onFinished firing once per run (OK / backdrop click / Escape — the fade starts immediately, the outro clip if any plays underneath). Honors prefers-reduced-motion (renders in full with a static crest + gradient, no video, and waits for an explicit dismiss).",
